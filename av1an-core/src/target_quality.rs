    ...
    fn encode_probe(&self, chunk: &Chunk, q: f32) -> Result<PathBuf, Box<EncoderCrash>> {
        ...
        let probe_result = thread::scope(move |scope| -> Result<(), Box<EncoderCrash>> {
            ...
            let source_pipe_status = source_pipe
                .as_mut()
                .map(|source_pipe| {
                    source_pipe.wait().map_err(|e| {
                        Box::new(EncoderCrash {
                            exit_status:        std::process::ExitStatus::default(),
                            source_pipe_stderr: String::new().into(),
                            ffmpeg_pipe_stderr: None,
                            stderr:             format!(
                                "Failed to wait for intermediate ffmpeg: {e}"
                            )
                            .into(),
                            stdout:             String::new().into(),
                        })
                ...
                .transpose()?;
            let source_status = source.wait().map_err(|e| {
                Box::new(EncoderCrash {
                    exit_status:        std::process::ExitStatus::default(),
                ...
                || !source_status.success()
            {
                let exit_status = if !enc_status.success() {
                    ...
                    *source_pipe_status.as_ref().expect("source pipe status was checked")
                } else {
                    source_status
                };
                return Err(Box::new(EncoderCrash {
                    ...
                    stdout: format!(
                        ...
                        source_pipe_status
                            .map_or_else(|| "not used".to_string(), |status| status.to_string())
                    )
                    .into(),
                }));
            }
